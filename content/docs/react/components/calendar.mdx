---
title: 'Calendar 日历'
description: '日历组件的日期渲染、自定义单元格、月份切换与国际化'
---
...
![alt text](/react/components/calendar-overview.png)
...
![alt text](/react/components/calendar-classname-and-control-mode.png)

使用 `classnames` 来处理类名的合并。[classnames 文档](https://www.npmjs.com/package/classnames)

## 日期渲染

![alt text](/react/components/calendar-date-render-1.png)
...
![alt text](/react/components/calendar-date-render-2.png)

### 自定义单元格内容

从父组件中接收 `value` `dateRender` `dateInnerContent` 为日期渲染做准备。

![alt text](/react/components/calendar-custom-cell-1.png)

子组件中继承父组件的 `props`。

![alt text](/react/components/calendar-custom-cell-2.png)
...
```
![alt text](/react/components/calendar-custom-cell-3.png)
...
```
![alt text](/react/components/calendar-date-select.png)
...
![alt text](/react/components/calendar-month-switch.png)
...
![alt text](/react/components/calendar-today-jump.png)
...
![alt text](/react/components/calendar-i18n-1.png)

在 `Calendar` 中注入国际化内容。随后使用时直接从 `useContext` 中获取即可。

![alt text](/react/components/calendar-i18n-2.png)

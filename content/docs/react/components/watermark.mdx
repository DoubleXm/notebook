---
title: 'WaterMark 水印'
description: 'useWaterMark、Canvas 绘制、合并配置与防删除的 MutationObserver'
---
...
![alt text](/react/components/watermark-hook-1.png)

![alt text](/react/components/watermark-hook-2.png)

可以看出整体流程比较清晰，除了 `children` 的所有属性都接收过来，同时通过自身维护一个 `options` 来监听变化并且绘制水印。

![alt text](/react/components/watermark-hook-3.png)

对于外部而言则是先将配置传入，作为首次的渲染，后续每次变化都会触发合并 `options` 来更新渲染。

#### 合并处理

![alt text](/react/components/watermark-merge-options.png)

#### 水印绘制

在 `container` 下面再创建一个 `div` 作为水印的载体，给他设置一堆水印相关样式并使用定位固定在页面上。

![alt text](/react/components/watermark-draw.png)
...
![alt text](/react/components/watermark-canvas-data-1.png)

如果是文字，用户又没有传入宽高，计算的方式则比较复杂。通过 `measureTextSize` 方法来计算文字的宽高，然后设置到 canvas 上。

同样的也要考虑行高，让文字在行高的一半位置（计算出坐标）开始绘制。

![alt text](/react/components/watermark-canvas-data-2.png)

获取到关键信息后，就可以针对这些信息开始文字的绘制了。

![alt text](/react/components/watermark-canvas-data-3.png)

### 效果

![alt text](/react/components/watermark-effect-1.png)

![alt text](/react/components/watermark-effect-2.png)
...
![alt text](/react/components/watermark-observer-1.png)

![alt text](/react/components/watermark-observer-2.png)

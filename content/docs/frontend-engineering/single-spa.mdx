...
description: '基座与 Vue3 / React 子应用接入方案，以及 single-spa 的加载原理'
---
...
一般用来加载子应用（通过 [SystemJS](/docs/frontend-engineering/module-system)）、数据通信、承接项目中的 `layout` 功能。脚手架创建出来的项目关注两个文件，分别为 `src/index.ejs`、`src/micro-web-root-config.js`。*`micro-web` 是组织名* 
...
![single-spa](/frontend-engineering/single-spa-application-status.png)
...
```js tab="application/app-helper.js"
// 加载流程
...
```js tab="lifecycles/bootstrap.js"

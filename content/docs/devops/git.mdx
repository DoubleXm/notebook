---
title: 'Git'
description: '初始化、SSH 配置、分支合并、冲突处理与 rebase'
---
...
![alt text](/devops/git-ssh-config.png)
...
![alt text](/devops/git-basic-workflow.png)
...
![alt text](/devops/git-merge-1.png)

`git log` 查看内容如下

![alt text](/devops/git-merge-2.png)

###  冲突合并

合并冲突时，命令行一般也会指出冲突的地方有几处，如下图。

![alt text](/devops/git-merge-conflict-1.png)

此时如果你的语言中不是 `>>>` 使用频率很高，可以考虑使用 `IDE` 的全局搜索，找到所有的冲突，挨个解决，自行选择这些冲突代码是否使用。如下图

![alt text](/devops/git-merge-conflict-2.png)
...
![alt text](/devops/git-rebase-1.png)
![alt text](/devops/git-rebase-2.png)

此时在 `main` 分支执行 `git rebase test`，没冲突的情况下直接提示 `Successfully rebased and updated refs/heads/main.`

再次查看 `log` 内容如下

![alt text](/devops/git-rebase-3.png)

### 冲突合并

当合并冲突时，警告信息提示的很明确你应当如何操作。

![alt text](/devops/git-rebase-conflict-1.png)

同样的，你也能在 `IDE` 中清楚的看到冲突的地方在哪里，自行选择如何合并这些代码。

![alt text](/devops/git-rebase-conflict-2.png)
...
git rebase -i HEAD~3
```
![alt text](/devops/git-squash-commits.png)

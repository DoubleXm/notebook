---
title: 'Charles 使用指南'
description: 'Charles 安装破解、SSL 配置、Chrome/移动端抓包与 Rewrite、BreakPoint 等常用功能'
---
...
![charles1](/journal/install-and-setup.png)
...
![charles2](/journal/ssl-proxy-settings-1.jpg)

- 导航栏中 `Proxy -> Proxy Settings` 增加如下配置；配置关于协议的内容。

![charles3](/journal/ssl-proxy-settings-2.jpg)

- 导航栏中 `Proxy -> Recording Settings` 增加如下配置；只有匹配下面的内容才会被记录在 `Charles` 中。

![charles4](/journal/ssl-proxy-settings-3.jpg)
...
![charles5](/journal/chrome-capture-1.jpg)

在 `auto switch` 中添加条件，并且将情景模式选择为 `Charles`，然后点击应用选项。具体效果如下：

![charles6](/journal/chrome-capture-2.jpg)

最后开启插件启用规则 `Charles` 或 `auto switch` 再开启抓包工具即可完成浏览器的抓包。具体效果如下：

![charles7](/journal/chrome-capture-3.jpg)
...
![charles8](/journal/android-capture.jpg)
...
![charles9](/journal/ios-capture.jpg)
...
![charles10](/journal/network-throttling.jpg)
...
![charles12](/journal/rewrite-1.jpg)
![charles13](/journal/rewrite-2.jpg)
![charles14](/journal/rewrite-3.jpg)
...
![charles15](/journal/breakpoint-1.jpg)
...
![charles16](/journal/breakpoint-2.jpg)
